...
export default function Requisition() {
  ...
  const columns = [
    ...
    { key: 'approvalStatus', label: 'Approval', render: (val) => (
      <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
        val === 'Approved' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700 animate-pulse'
      }`}>
        {val}
      </span>
    )},
  ...
  const actions = (row) => row.approvalStatus === 'Pending Approval' ? (
    <button 
      onClick={() => {
        approveOrder(row.id);
        addNotification(`PR ${row.transactionId} released successfully`, 'success');
      }}
